...
test('waits for a successful receipt and explains common wallet failures', async () => {
  ...
  expect(friendlyTransactionError(new Error('External transactions to internal accounts cannot include data'))).toMatch(/MetaMask-compatible ESIP-3 creation path/i);
});
