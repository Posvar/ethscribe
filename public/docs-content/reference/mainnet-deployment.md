...
> **Deployment record:** V2 is immutable and exact-match source-verified. The release includes direct creation, registered deposits, fixed-price trading, and exits for both custody forms. No independent security audit is documented. Activation is recorded below; operational availability depends on current contract and service state.
...
| Recorded activation state | Active at activation block; query current state before preparing a transaction |
...
The canonical machine-readable record is [`contracts/deployments/mainnet-v2.json`](https://github.com/Posvar/ethscribe/blob/main/contracts/deployments/mainnet-v2.json). It records an exact runtime match against the artifact built from the frozen source commit.
...
Each batch method accepts at most 100 IDs. A wallet containing both forms uses two batch transactions, one for each form. This separation prevents a direct-exit call from bypassing an active listing or registered deposit state. Both exit paths remain callable while the market is paused. The current Field Wallet UI exposes individual withdrawals; these batch functions are contract capabilities.
...
Source verification establishes that deployed code matches published source. It is not an independent security audit or evidence that every UI transaction path has been exercised end to end.
