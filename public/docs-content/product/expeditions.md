...
A **known-file target** has a reference digest established from preserved source material. The hunter recovers a candidate; the site can test exact equality before preparing a transaction.

A **lost-file target** has historical evidence of existence but no established original bytes. It needs a provenance argument. Matching a filename, format, or dimensions cannot settle the claim.

The lost-file target remains unresolved until historical review supports a recovery.

## The current participation loop

1. Explore a target and follow its historical clues.
2. Test an uploaded candidate or an existing Ethscription.
3. Create or deposit a qualifying asset into market custody.
4. Sign and publish the target assignment.
5. Inspect the recovered record, keep or withdraw the artifact, or list it for sale.

Creation, assignment, and sale are separate actions. [Your first Finding](first-finding.md) explains each step.
...
The count is not a protocol-wide search for every earlier raw-byte match under another wrapper.
...
## Proposals and future expeditions
...
The intended long-term archive retains the target, evidence, recognized Ethscription, and corrections even after ownership changes. Today, Ethscription history is recorded on Ethereum while the catalogue and signed assignments depend on the public repository and hosted storage. Redundant publication and richer correction history remain development work.
